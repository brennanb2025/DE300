...
def print_data_filtered_by_age(data: DataFrame):
    """
    select rows with 'age' between 30 and 50 (inclusive) and transforms the selected pyspark dataframe into pandas dataframe 
    and print out the summary statistics using 'describe()'
    """
    
    local_data = data.filter((data['age'] >= 30) & (data['age'] <= 50))
    local_data_pandas = local_data.toPandas()

    print(local_data_pandas.describe())
    
    return local_data_pandas


def save_spark_dataframe_to_csv(data: DataFrame):
    """
    Save dataframe to a csv file in ./data. Note: WITHOUT THE STEP OF FILTERING ABOVE.
    "When you run 'bash run-py-spark.sh', the .py file should be executed with Spark 
    (i.e including the steps of data reading data, data cleaning, data Transformation, 
    but without the step in task 1 above), and the final dataframe should be stored as .csv file in the './data' folder."
    
    """
    # data.write.csv("./data/spark_dataframe.csv", header=True, mode="overwrite")
    # This line is saving the csv file as multiple files. I want just one.
    # I'm going to convert it to a pandas dataframe before I save it.
    data.toPandas().to_csv('./data/spark_dataframe.csv', index=False)
...
def main():
    ...
    pandas_df = print_data_filtered_by_age(data)

    save_spark_dataframe_to_csv(data)
