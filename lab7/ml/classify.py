...
def read_data(spark: SparkSession) -> DataFrame:
    ...
    titanic_data = spark.read.csv("s3://brennanbensonawsbucket/data/data.csv", header=True, inferSchema=True)
...
def pipeline(data: DataFrame):
    ...
    # Create all squared numeric features (1)
    square_columns = [f"{col}_squared" for col in imputed_columns_numeric]
    square_features = SquaredFeatures(inputCols=imputed_columns_numeric, outputCols=square_columns)

    # Assemble feature columns into a single feature vector
    assembler = VectorAssembler(
        inputCols=pairwise_columns + square_columns + imputed_columns_numeric + imputed_columns_string, 
        outputCol="features"
        )
